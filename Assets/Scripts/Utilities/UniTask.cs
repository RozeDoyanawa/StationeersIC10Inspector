...
namespace ridorana.IC10Inspector.Utilities {
  public static class UniTaskX
  {
    ...
    private delegate UniTask NextFrame1(CancellationToken cancellationToken);
    private delegate UniTask NextFrame2(CancellationToken cancellationToken, bool cancelImmediately);
    private static NextFrame1 NextFrameFn;
    public static UniTask NextFrame(CancellationToken cancellationToken)
    {
      static NextFrame1 wrap(NextFrame2 fn) => c => fn(c, false);
      NextFrameFn ??= MakeCompatDelegate<NextFrame1, NextFrame2>("NextFrame", wrap);
      return NextFrameFn(cancellationToken);
    }
